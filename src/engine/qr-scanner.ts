...
export async function scanSlipQR(
  imageBase64: string,
): Promise<QRSlipData | null> {
  try {
    ...
    console.warn("[qr-scanner] scan failed:", (err as Error).message);
    return null;
  }
}
...
function parseEMVCo(qrData: string): QRSlipData {
  const result: QRSlipData = { amount: null, refNumber: null, bankCode: null };
  ...
  try {
    ...
    while (i + 4 <= qrData.length) {
      ...
      if (isNaN(len) || i + 4 + len > qrData.length) break;
      ...
      if (tag === "62") {
        ...
        while (j + 4 <= value.length) {
          ...
          if (subTag === "05") result.refNumber = subValue;
        }
      }
      ...
      if (tagNum >= 26 && tagNum <= 51) {
        ...
        while (j + 4 <= value.length) {
          ...
          j += 4 + subLen;
          if (subTag === "00") {
            if (subValue.toLowerCase().includes("promptpay")) result.bankCode = "PromptPay";
            else if (subValue.includes("004")) result.bankCode = "KBank";
            else if (subValue.includes("014")) result.bankCode = "SCB";
            else if (subValue.includes("002")) result.bankCode = "BBL";
            else if (subValue.includes("006")) result.bankCode = "Krungthai";
          }
        }
      }
    }
