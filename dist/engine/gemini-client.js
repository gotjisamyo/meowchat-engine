...
exports.analyzeSlipImage = analyzeSlipImage;
exports.routeModel = routeModel;
exports.callGemini = callGemini;
const generative_ai_1 = require("@google/generative-ai");
// ─── Analyze slip image with Gemini Vision ────────────────────────────────────
async function analyzeSlipImage(imageBase64, mimeType, apiKey) {
...
function routeModel(userMessage, defaultModel) {
    const isSimple = userMessage.length < 40 ||
        /^(สวัสดี|ขอบคุณ|โอเค|ok|hi|hello|ราคา|มีไหม|เปิดไหม)$/i.test(userMessage.trim());
...
async function callGemini(payload, apiKey) {
    const genAI = new generative_ai_1.GoogleGenerativeAI(apiKey);
    const modelName = routeModel(payload.messages.at(-1)?.content ?? "", payload.model);
