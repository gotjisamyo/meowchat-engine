...
exports.setupPlatformRichMenu = setupPlatformRichMenu;
...
const RICH_MENU = {
    ...
    areas: [
        ...
        },
        {
            ...
            action: { type: "uri", label: "ทดลองฟรีสิบสี่วัน", uri: "https://my.meowchat.store/register" },
        },
        {
...
async function lineGet(path, token) {
    ...
    if (!res.ok)
        throw new Error(`LINE GET ${path} → ${res.status}`);
    return res.json();
}
async function linePost(path, token, body) {
    ...
    if (!res.ok)
        throw new Error(`LINE POST ${path} → ${res.status}: ${text}`);
    return JSON.parse(text);
}
// ─── Main export ──────────────────────────────────────────────────────────────
async function setupPlatformRichMenu(accessToken) {
    try {
        // 1. Check if default rich menu already exists
        const existing = (await lineGet("/v2/bot/user/all/richmenu", accessToken).catch(() => null));
        ...
        const tmpDir = node_path_1.default.join(__dirname, "../../.tmp");
        (0, node_fs_1.mkdirSync)(tmpDir, { recursive: true });
        const imgPath = node_path_1.default.join(tmpDir, "platform-richmenu.png");
        const pyScript = node_path_1.default.join(__dirname, "../../scripts/generate-richmenu-image.py");
        const gen = (0, node_child_process_1.spawnSync)("python3", [pyScript, imgPath], { encoding: "utf8" });
        if (gen.status !== 0) {
            throw new Error(`Image gen failed: ${gen.stderr || gen.stdout}`);
        }
        console.log("[richmenu]", gen.stdout.trim());
        // 3. Create rich menu structure
        const created = (await linePost("/v2/bot/richmenu", accessToken, RICH_MENU));
        const richMenuId = created.richMenuId;
        console.log(`[richmenu] Created: ${richMenuId}`);
        // 4. Upload image
        const imgBuf = (0, node_fs_1.readFileSync)(imgPath);
        ...
        // Cleanup
        try {
            (0, node_fs_1.unlinkSync)(imgPath);
        }
        catch { /* ignore */ }
    }
    catch (err) {
        // Non-fatal: log and continue (bot works without rich menu)
        console.warn("[richmenu] Setup failed (non-fatal):", err.message);
    }
}
