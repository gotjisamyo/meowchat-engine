...
exports.processPlatformEvent = processPlatformEvent;
const redis_js_1 = require("../memory/redis.js");
const platform_demo_js_1 = require("../engine/platform-demo.js");
...
async function notifyGotAndCreateHandoff(userId, userText, token) {
...
async function getState(userId) {
    try {
        const raw = await redis_js_1.redis.get(`platform:state:${userId}`);
...
async function setState(userId, state) {
    try {
        ...
        await redis_js_1.redis.set(`platform:state:${userId}`, JSON.stringify(state), "EX", STATE_TTL);
    }
    catch {
...
function imgMsg(filename) {
    const url = `${ASSET_BASE}/${filename}`;
    return { type: "image", originalContentUrl: url, previewImageUrl: url };
}
function textMsg(text, quickReplies) {
...
function buildReply(replyToken, text, quickReplies) {
    return { replyToken, messages: [textMsg(text, quickReplies)] };
}
function buildReplyWithImages(replyToken, images, text, quickReplies) {
...
function buildFlexReply(replyToken, flex, extra) {
    const messages = [flex];
    if (extra)
        messages.push(extra);
    return { replyToken, messages };
...
async function sendPush(userId, messages, accessToken) {
...
async function sendReply(payload, accessToken) {
...
async function processPlatformEvent(event, config) {
    ...
    if (isEscalation(text)) {
        await sendReply(buildReply(replyToken, "ขอบคุณที่ติดต่อมานะคะ 🙏 ทีมงาน MeowChat จะติดต่อกลับโดยเร็วที่สุดค่ะ!\n\nสามารถทิ้งข้อมูลเบื้องต้นไว้ได้เลย เช่น ประเภทธุรกิจ หรือคำถามที่ต้องการ", [
    ...
    if (t === "ติดต่อทีม" || t === "คุยกับทีมงาน") {
        await sendReply(buildReply(replyToken, CONTACT_MESSAGE, [
            { label: "🚀 ทดลองฟรี 14 วัน", text: "ทดลองฟรี" },
            { label: "💰 ดูราคา", text: "ราคา" },
        ]), token);
        notifyGotAndCreateHandoff(userId, text, token).catch(() => { });
    ...
    if (t === "ดูตัวอย่าง" || t.includes("demo")) {
        ...
        if (bizType) {
            await sendReply(buildReplyWithImages(replyToken, ["chatdemo.jpg", "usecases.jpg"], (0, platform_demo_js_1.getDemoMessage)(bizType), [
            ...
            await setState(userId, state);
            await sendReply(buildReply(replyToken, `ขอถามก่อนนะคะ เพื่อให้ดูตัวอย่างที่ตรงกับธุรกิจของคุณ 😊\n\nร้านหรือธุรกิจของคุณเป็นประเภทไหนคะ?`, platform_demo_js_1.WELCOME_QUICK_REPLIES), token);
        }
        return;
    }
    // ── New user ───────────────────────────────────────────────────────────────
    if (!state || state.stage === "new") {
        ...
        await setState(userId, state);
        await sendReply(buildReply(replyToken, platform_demo_js_1.WELCOME_MESSAGE, platform_demo_js_1.WELCOME_QUICK_REPLIES), token);
        return;
    }
    // ── Waiting for business type ─────────────────────────────────────────────
    if (state.stage === "asked_type") {
        const bizType = (0, platform_demo_js_1.detectBusinessType)(text);
        const resolvedType = bizType ?? "other";
        state.businessType = resolvedType;
        state.stage = "demo_shown";
        await setState(userId, state);
        await sendReply(buildReply(replyToken, (0, platform_demo_js_1.getDemoMessage)(resolvedType), [
    ...
    if (state.stage === "demo_shown") {
        ...
        await setState(userId, state);
        await sendReply(buildReply(replyToken, (0, platform_demo_js_1.getCTAMessage)(bizType), [
            { label: "✅ สมัครเลย!", text: "ทดลองฟรี" },
            { label: "💰 ดูราคา", text: "ราคา" },
    ...
    try {
        const result = await model.generateContent(`${MEOWCHAT_SYSTEM_PROMPT}\n\n=== ลูกค้าถาม ===\n${text}`);
        const reply = result.response.text().trim();
        await sendReply(buildReply(replyToken, reply, [
            { label: "🚀 ทดลองฟรี 14 วัน", text: "ทดลองฟรี" },
            { label: "💰 ดูราคา", text: "ราคา" },
            { label: "📞 คุยกับทีม", text: "ติดต่อทีม" },
        ]), token);
        ...
        await sendReply(buildReply(replyToken, "ขอโทษค่ะ ระบบมีปัญหาชั่วคราว กรุณาลองใหม่อีกครั้ง หรือติดต่อทีมงานได้เลยที่ LINE: @meowchat ค่ะ 🐱", [{ label: "📞 ติดต่อทีม", text: "ติดต่อทีม" }]), token);
    }
}
