...
exports.listBotIds = listBotIds;
exports.saveBotConfig = saveBotConfig;
exports.registerDemoBot = registerDemoBot;
exports.registerPlatformBot = registerPlatformBot;
...
async function getBotConfig(botId) {
    // 1. Try Redis first
    try {
        const raw = await redis_js_1.redis.get(`botconfig:${botId}`);
        if (raw) {
        ...
        console.warn("[registry] Redis get failed, using memCache:", err.message);
    }
...
async function saveBotConfig(config) {
    ...
    try {
        await redis_js_1.redis.set(`botconfig:${config.botId}`, JSON.stringify(config));
        console.log(`[registry] saved botId=${config.botId} to Redis + memCache`);
    }
    catch (err) {
        console.warn(`[registry] Redis set failed for botId=${config.botId}, memCache only:`, err.message);
    }
}
// ─── Example: register a demo bot (call this once during setup) ───────────────
async function registerDemoBot() {
    const demo = {
        botId: "demo-bot-001",
        botName: "น้องแมว",
        ...
        knowledgeBase: [
            {
                id: "menu-001",
                topic: "เมนูอาหาร",
                content: "ข้าวผัดหมู ฿80 · ข้าวกะเพราไข่ดาว ฿75 · ข้าวมันไก่ ฿70 · " +
            ...
            },
            {
                id: "delivery-001",
                topic: "จัดส่ง",
                content: "จัดส่งในรัศมี 5 กม. ค่าส่ง ฿20 หรือฟรีเมื่อสั่งครบ ฿200 " +
            ...
            },
            {
                id: "hours-001",
                topic: "เวลาทำการ",
                content: "เปิดทุกวัน 08:00–21:00 น. หยุดวันพุธ " +
            ...
            },
            {
                id: "promo-001",
                topic: "โปรโมชั่น",
                content: "ลูกค้าใหม่รับส่วนลด 10% เมื่อสั่งครั้งแรก ใช้โค้ด NEWMAE " +
...
async function registerPlatformBot() {
